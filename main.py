...
def get_data(isin, source=“de”):
“”“Stáhne cenu a změnu z Tradegate. Vrací (price, change, error).”””
try:
# Výběr správné URL podle zdroje
if source == “bsx”:
url = tradegate_bsx_url(isin)
else:
url = tradegate_url(isin)
...
for ticker, (name, isin, source) in ETFS.items():
    price, change, error = get_data(isin, source)
    if error:
